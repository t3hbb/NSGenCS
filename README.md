...
Want to do fileless? Just have an empy `SHELLCODEHERE` and use `encrypt` to create a download function to grab your payload from a remote host (or over SMB if you want). Again make sure that your delivery template has the appropriate `using xxxxxxxxxx`.
